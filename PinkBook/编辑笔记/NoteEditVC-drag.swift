...
extension NoteEditVC: UICollectionViewDropDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, dropSessionDidUpdate session: UIDropSession, withDestinationIndexPath destinationIndexPath: IndexPath?) -> UICollectionViewDropProposal {
        ...
        if collectionView.hasActiveDrag { // 当用户在拖拽时, 设置是拖拽状态, 提升performDropWith的性能
            return UICollectionViewDropProposal(operation: .move, intent: .insertAtDestinationIndexPath)
        }
        return UICollectionViewDropProposal(operation: .forbidden) // 非拖拽就关闭
    }
    ...
    func collectionView(_ collectionView: UICollectionView, performDropWith coordinator: UICollectionViewDropCoordinator) {
        ...
        if coordinator.proposal.operation == .move, // 拖拽状态
           let item = coordinator.items.first, // 本例中拖拽单个，故获取first即可
           let sourceIndexPath = item.sourceIndexPath, // 拖拽cell的原始下标
           let destinationIndexPath = coordinator.destinationIndexPath // 拖拽cell的目标下标
        {
            // 将多个操作合并为一个动画
            collectionView.performBatchUpdates {
                photos.remove(at: sourceIndexPath.item) // 先删除
                photos.insert(item.dragItem.localObject as! UIImage, at: destinationIndexPath.item) // 在插入
                collectionView.moveItem(at: sourceIndexPath, to: destinationIndexPath) // 刷新
            }
            // 固定操作,让拖拽变得自然
